...
@click.option('--graph-repo', default=None, help='Path to graph repository (overrides .env)')
@click.option('--docs-path', default=None, help='Path to documentation (overrides .env)')
@click.option('--run', is_flag=True, default=False, help='Execute the top recommended graph (requires ALLOW_EXECUTION=true)')
@click.option('--explain', is_flag=True, default=False, help='Show detailed explanation for each recommendation')
...
@click.option('--graph-repo', default=None, help='Path to graph repository (overrides .env)')
@click.option('--run', is_flag=True, default=False, help='Execute the graph (requires ALLOW_EXECUTION=true)')
def execute(graph_name: str, graph_repo: Optional[str], run: bool):
    ...
    if not graph:
        ...
        click.echo(f"Available graphs: {', '.join([g['name'] for g in catalog.get_all_graphs()[:10]])}")
        return
...
def status():
    ...
    click.echo(f"\nSafety status:")
    if allow_execution:
        click.echo(f"  ⚠ Execution is ENABLED")
        if not dynamo_cli or not cli_exists:
            click.echo(f"  ⚠ But Dynamo CLI is not configured or not found")
    else:
        click.echo(f"  ✓ Execution is DISABLED (safe)")
    
    click.echo(f"\nTo enable execution:")
    click.echo(f"  1. Set ALLOW_EXECUTION=true in .env")
    click.echo(f"  2. Set DYNAMO_CLI_PATH to your Dynamo executable in .env")
    click.echo(f"  3. Use --run flag when running recommend or execute commands")
